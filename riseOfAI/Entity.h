...
enum AIType     { WALKER, GUARD, MISSILE, CROW  };
enum AIState    { WALKING, IDLE, BUFFER, ATTACKING };
...
public:
    ...
    // Pounce mechanic
    bool m_is_pouncing     = false;
    float m_pounce_speed = 0;
    ...
    void draw_sprite_from_texture_atlas(ShaderProgram *program, GLuint m_texture_id, int index);
    ...
    void ai_crow(Entity* player);
    void ai_missile(Entity* player);
    ...
    glm::vec3  const get_m_movement()     const { return m_movement;     };
    ...
    void const set_enemy_num(int new_enemies) { enemy_num = new_enemies; };
};
